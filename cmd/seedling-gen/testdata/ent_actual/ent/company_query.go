...
func (_q *CompanyQuery) loadUsers(ctx context.Context, query *UserQuery, nodes []*Company, init func(*Company), assign func(*Company, *User)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(user.FieldCompanyUUID)
	}
	...
	for _, n := range neighbors {
		fk := n.CompanyUUID
		if fk == nil {
			return fmt.Errorf(`foreign-key "company_uuid" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "company_uuid" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
