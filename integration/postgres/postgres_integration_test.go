...
import (
	...
	"github.com/mhiro2/seedling/integration/postgres/testutil"
...
func TestPostgresInsertOne_PersistsResolvedDependencies(t *testing.T) {
	// Arrange
...
func TestPostgresInsertManySeq_PersistsGeneratedNames(t *testing.T) {
	// Arrange
	h := testutil.NewHarness(t)
	db := h.DB
	reg := h.Registry
...
func TestPostgresInsertOneE_ReturnsForeignKeyViolation(t *testing.T) {
	// Arrange
	h := testutil.NewHarness(t)
	db := h.DB
	reg := h.Registry
...
func TestPostgresInsertOneE_ReturnsConnectionError(t *testing.T) {
	// Arrange
	h := testutil.NewHarness(t)
	db := h.DB
	reg := h.Registry
...
func TestPostgresInsertOne_RollsBackTransaction(t *testing.T) {
	// Arrange
	h := testutil.NewHarness(t)
	db := h.DB
	reg := h.Registry
...
func TestPostgresInsertOne_HasManyPersistsResolvedDependencies(t *testing.T) {
	// Arrange
	h := testutil.NewHarness(t)
	db := h.DB
	reg := h.Registry
...
func TestPostgresInsertOne_CompositePKPersistsResolvedDependencies(t *testing.T) {
	// Arrange
	h := testutil.NewHarness(t)
	db := h.DB
	reg := h.Registry
...
func TestPostgresInsertOne_ManyToManyPersistsResolvedDependencies(t *testing.T) {
	// Arrange
	h := testutil.NewHarness(t)
	db := h.DB
	reg := h.Registry
	...
	rows, err := db.Query(`SELECT tag_id FROM article_tags WHERE article_id = $1 ORDER BY tag_id`, inserted.ID)
	if err != nil {
